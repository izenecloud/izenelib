...
static int
euc_jp_mbtowc (ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  ...
  if (c == 0x8e) {
      ...
      return RET_TOOFEW(0);
    {
      unsigned char c2 = s[1];
      if (c2 >= 0xa1 && c2 < 0xe0) {
        int ret = jisx0201_mbtowc(pwc,s+1,n-1);
        if (ret == RET_ILSEQ)
          return RET_ILSEQ;
        if (ret != 1) abort();
        return 2;
...
static int
euc_jp_wctomb (unsigned char *r, ucs4_t wc, int n)
{
  unsigned char buf[2];
  int ret;
