...
#ifndef SIMD_COMPRESSION_DELTATEMPLATES_H_
#define SIMD_COMPRESSION_DELTATEMPLATES_H_
...
/**
 * The structs NoDelta, RegularDeltaSIMD, CoarseDelta4SIMD, CoarseDelta2SIMD, Max4DeltaSIMD
...
struct CoarseDelta2SIMD {
    ...
    __attribute__((always_inline))
    static inline __m128i Delta(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(curr, (__m128i)_mm_shuffle_pd((__m128d)prev, (__m128d)curr, _MM_SHUFFLE2(0, 1)));
    }
    static bool usesDifferentialEncoding() { return true; }
...
struct ReverseRegularDeltaSIMD {
    // Folklore code, unknown origin of this idea
    __attribute__((always_inline))
    static inline __m128i PrefixSum(__m128i curr, __m128i prev) {
        const __m128i _tmp1 = _mm_add_epi32(_mm_slli_si128(curr, 8), curr);
        const __m128i _tmp2 = _mm_add_epi32(_mm_slli_si128(_tmp1, 4), _tmp1);
        return _mm_sub_epi32(_mm_shuffle_epi32(prev, 0xff), _tmp2);
    }

    __attribute__((always_inline))
    static inline __m128i Delta(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(_mm_or_si128(_mm_slli_si128(curr, 4), _mm_srli_si128(prev, 12)), curr);
    }

    static bool usesDifferentialEncoding() { return true; }

    static std::string name() { return "ReverseDelta1"; }
};

struct ReverseCoarseDelta4SIMD {
    __attribute__((always_inline))
    // Proposed and implemented by L. Boytosv
    static inline __m128i PrefixSum(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(prev, curr);
    }
    __attribute__((always_inline))
    static inline __m128i Delta(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(prev, curr);
    ...
    static std::string name() { return "ReverseDelta4"; }
};

struct ReverseCoarseDelta2SIMD {
    __attribute__((always_inline))
    // Proposed and implemented by L. Boytosv
    static inline __m128i PrefixSum(__m128i curr, __m128i prev) {
        const __m128i _tmp1 = _mm_add_epi32(_mm_slli_si128(curr, 8), curr);
        return _mm_sub_epi32(_mm_shuffle_epi32(prev, _MM_SHUFFLE(3, 2, 3, 2)), _tmp1);
    }
    __attribute__((always_inline))
    static inline __m128i Delta(__m128i curr, __m128i prev) {
        return _mm_sub_epi32((__m128i)_mm_shuffle_pd((__m128d)prev, (__m128d)curr, _MM_SHUFFLE2(0, 1)), curr);
    }
    static bool usesDifferentialEncoding() { return true; }

    static std::string name() { return "ReverseDelta2"; }
};

struct ReverseMax4DeltaSIMD {
    __attribute__((always_inline))
    // The idea is due to N. Kurz
    static inline __m128i PrefixSum(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(_mm_shuffle_epi32(prev, 0xff), curr);
    }
    __attribute__((always_inline))
    static inline __m128i Delta(__m128i curr, __m128i prev) {
        return _mm_sub_epi32(_mm_shuffle_epi32(prev, 0xff), curr);
    }
    static std::string name() { return "ReverseDeltaM4"; }
