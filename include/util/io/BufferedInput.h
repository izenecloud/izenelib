...
namespace io{
///BufferedInput is a class that is responsible for the index data reading
///It provides basic utilites of decoding variable length data
class BufferedInput
{
public:
    BufferedInput(char* buffer,size_t buffsize);

    BufferedInput(size_t buffsize=0);

    virtual ~BufferedInput(void);
public:
...
//
inline uint8_t BufferedInput::readByte()
{
    if (bufferPosition_ >= bufferLength_)
        refill();
    return buffer_[bufferPosition_++];
}
inline int32_t BufferedInput:: readInt()
{
    uint8_t b1 = readByte();
    uint8_t b2 = readByte();
    uint8_t b3 = readByte();
...
inline int32_t BufferedInput::readVInt()
{
    uint8_t b = readByte();
    int32_t i = b & 0x7F;
...
inline int64_t BufferedInput::readLong()
{
    int32_t i1 = readInt();
    int32_t i2 = readInt();
...
inline int64_t BufferedInput::readVLong()
{
    uint8_t b = readByte();
    int64_t i = b & 0x7F;
    for (int32_t shift = 7; (b & 0x80) != 0; shift += 7)
...
inline void BufferedInput::readString(std::string& s)
{
...
inline void BufferedInput::readChars(char* buffer, size_t start, size_t length)
{
...
/////////////////////////////Small Endian/////////////////////////////////////////////
//
inline int32_t BufferedInput:: readIntBySmallEndian()
{
    uint8_t b1 = readByte();
    uint8_t b2 = readByte();
    uint8_t b3 = readByte();
...
inline int32_t BufferedInput::readVIntByBigEndian()
{
    uint8_t b[5];
    b[0] = readByte();
    uint8_t i = 0, shift = 0;
...
inline int64_t BufferedInput::readLongBySmallEndian()
{
...
inline void BufferedInput::refill()
{
    if(dirty_)
    {
...
inline int64_t BufferedInput::getFilePointer()
{
    return bufferStart_ + (int64_t)bufferPosition_;
}

inline void BufferedInput::seek(int64_t pos)
{
    if (pos > length_)
        throw std::runtime_error(
            "BufferedInput.seek():pos>length_");
...
inline bool BufferedInput::isEof()
{
    return ( (bufferStart_ + (int64_t )bufferPosition_) >= length_);
}

inline int64_t BufferedInput::length()const
{
    return length_;
}

inline void BufferedInput::setlength(int64_t newLen)
{
    length_ = newLen;
}
