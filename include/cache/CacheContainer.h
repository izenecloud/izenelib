...
*/	
   bool find(const KeyType& key){return CacheInfoHash_.find(key) != CacheInfoHash_.end();}
/**
*	\brief It gets the first item, i.e the item with lowest priority.
*/
	MIT begin(){return keyInfoMap_.begin();}
/**
*	\brief It gets the last item.
*/
	MIT end(){return keyInfoMap_.end();}
/**
*	\brief It finds the next item.
*/
	MIT next(MIT& it){return ++it; }
/**
*	\brief It determines  if an item is out of date in CacheContainer. 
*/
	bool isOutOfDate(const KeyType& key){return keyInfoMap_[CacheInfoHash_[key]];}
