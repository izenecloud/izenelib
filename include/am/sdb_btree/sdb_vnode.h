...
bool fixed = false,
typename Alloc=std::allocator<DataType<KeyType,ValueType> > > class sdb_vnode_ {
	typedef std::pair<sdb_vnode_*, size_t> NodeKeyLocn;
...
public:
	 ...
	 */
	sdb_vnode_(CbFileHeader& fileHeader, LockType& fileLock, size_t& activeNum);
	 ...
	 */
	inline sdb_vnode_* loadChild(size_t childNum, FILE* f);
...
public:
	...
	std::vector<sdb_vnode_*> children; //it has objCount+1 childrens.
...
template<typename KeyType, typename ValueType, typename LockType, bool fixed,
		typename Alloc> CompareFunctor<KeyType> sdb_vnode_< KeyType, ValueType,
		LockType, fixed, Alloc>::_comp;

template<typename KeyType, typename ValueType, typename LockType, bool fixed,
		typename Alloc> sdb_vnode_< KeyType, ValueType, LockType, fixed, Alloc>::sdb_vnode_(
...
template<typename KeyType, typename ValueType, typename LockType, bool fixed,
		typename Alloc> bool sdb_vnode_< KeyType, ValueType, LockType, fixed,
		Alloc>::read(FILE* f) {
	...
	if (objCount> 0 && !isLeaf) {
		...
		if ( !isLeaf) {
			for (size_t i = 0; i < objCount+1; i++) {
				if (children[i] == 0) {
					children[i] = new sdb_vnode_(_fh, _fileLock, activeNodeNum );
				}
	...
	p += sizeof(size_t);
	izene_deserialization< std::vector<ValueType> > izd1(p, vsize);
	izd1.read_image(values);
...
template<typename KeyType, typename ValueType, typename LockType, bool fixed,
		typename Alloc> void sdb_vnode_< KeyType, ValueType, LockType, fixed,
		Alloc>::unload() {
	if (isLoaded) {
		if ( !isLeaf) {
			for (size_t i=0; i<objCount+1; i++) {
...
template<typename KeyType, typename ValueType, typename LockType, bool fixed,
		typename Alloc> KEYPOS sdb_vnode_< KeyType, ValueType, LockType, fixed,
