...
private:
	char* getBlock(long fpos, bool isLoad){
		...
		if ( !mbList[n]) {
			...
			if( isLoad )mbList[n]-> load(_dataFile);			
		}		
...
template<typename KeyType, typename DataType, typename LockType, typename Alloc> bool BTreeNode<
	...
	for (size_t ctr = 0; ctr < objCount; ctr++) {
		...
		/*	int n = (_overFlowAddress - sizeof(SFileHeader) )/_pageSize;		
			...
			readOverFlow(_overFlowAddress, pBuf+_maxDataSize, recSize - _maxDataSize);
...
template<typename KeyType, typename DataType, typename LockType, typename Alloc> bool BTreeNode<
	...
	for (size_t ctr=0; ctr<objCount; ctr++) {
		...
		//	assert(false);
			...
			writeOverFlow(_overFlowAddress, pd+_maxDataSize, nopage*_overFlowSize);
	...
	/*for (map<size_t, OverflowInfo>::iterator it = _overflowMap.begin(); it
		...
		char *pbuf;
		pbuf = new char[it->second.second * _overFlowSize];
		memcpy(pbuf, pd + _maxDataSize, sz- _maxDataSize);

		long overFlowAddress = it->second.first;
		
		writeOverFlow(overFlowAddress, pbuf, it->second.second*_overFlowSize);
		...
		/*int n = (overFlowAddress - sizeof(SFileHeader) )/_pageSize;
		...
		memcpy(mbList[n]->getP(fpos), pbuf, it->second.second*_overFlowSize)*/		
...
template<typename KeyType, typename DataType, typename LockType, typename Alloc> intrusive_ptr<BTreeNode<KeyType, DataType,LockType, Alloc> > BTreeNode<
		KeyType, DataType, LockType, Alloc>::loadChild(size_t childNum,
		FILE* f, const string& fileName) {
