/*-----------------------------------------------------------------------------
 *  lzend.hpp - A LZ77-like compression algorithm with random accesses
 *
 *  Coding-Style:
...
#ifndef __LZEND_HPP__
#define __LZEND_HPP__
...
#define __USE_PROGRESS_INDICATOR__

#ifdef __USE_PROGRESS_INDICATOR__
 #include <time.h>
 #include <sys/time.h>
 ...
 #define PROG_PER_COUNT  10000000
 ...
 #define __show_progress(str, c, n)     \
        ({                      \
                double  pg;     \
                double  left;   \
 \
                if (c == 0 || !__init_progress++) {     \
                        __progress_count = 0;           \
                        __progress_start_time =         \
                                __progress_get_time();  \
                }       \
 \
                if (c != 0 && (__progress_count++ %             \
                                ((n <= PROG_PER_COUNT)? 1 :     \
                                n / PROG_PER_COUNT)) == 0) {    \
                        pg = (double)c / n;                     \
                        left = ((1.0 - pg) / pg) *              \
...
#else
 #define __show_progress(str, c, n)
#endif /* __USE_PROGRESSS_INDICATOR__ */
...
public:
        sufarray_t(const lzchar_t *text,
...
public:
        bws_t(const lzchar_t *text,
                ...
                ret = sais_bwt(static_cast<const unsigned char *>(text),
                                b, A, static_cast<int>(size));
                if (ret < 0)
                        LOG(FATAL) << "Can't use a sais library correctly";
...
public:
        ...
        bool is_source(range_t r,
                        uint32_t len, uint32_t& qidx) {
                uint32_t        val;
                pattern_it_t    it;
...
private:
        ...
        void build_lzend(const lzchar_t *text,
                ...
                while (idx < size) {
                        ...
                        range_t         r;
                         ...
                         __show_progress("build_lzend", idx, size);
        ...
        void append_lzend(const lzchar_t *text,
                        uint64_t size) {
                uint64_t        idx;
                uint64_t        base;
                ...
                while (idx < size) {
                        ...
                        range_t         r;
                         ...
                         __show_progress("append_lzend", idx, size);
