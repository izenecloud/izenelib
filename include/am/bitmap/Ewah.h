...
public:
    ...
    EWAHBoolArraySparseIterator(const EWAHBoolArraySparseIterator<uword> & other)
    :i(other.i)
...
public:
    ...
    uint getUncompressedSize() const
    {
        return totalliteral+ totalcompressed;
    }
    uint getNumberOfDirtyWords() const
    {
        return totalliteral;
    }
    uint getNumberOfCleanWords() const
    {
        return totalcompressed;
    }
    uint getNumberOfMarkers() const
    {
        return runningwordmarker;
    }
    uint getOverRuns() const
    {
...
public:
    ...
    void set(uint i)
    {
        ...
        //cout<<"sizeinbits = "<<sizeinbits<<endl;
...
public:
    ...
    : pointer(0)
    , myparent(&p.getBuffer())
    , rlw((*myparent)[pointer])   //RunningLength(0), NumberOfLiteralWords(0), Bit(0) {
    {
        if (verbose)
        {
...
void EWAHBoolArray<uword>::append(const EWAHBoolArray & x)
{
    ...
    else
    {
        ...
        ss<<"This should really not happen! You are trying to append to a bitmap having a fractional number of words, that is,  "
           <<static_cast<int>(sizeinbits)<<" bits with a word size in bits of "<<static_cast<int>(wordinbits)<<". ";
...
template <class uword>
void EWAHBoolArrayBitIterator<uword>::readNewWord()
{
    while (i.hasNext())
    {
        currentword = i.next();
