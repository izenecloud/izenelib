...
void* hlcalloc(size_t, size_t);

void* hlrealloc(void*, size_t);

void* hlmemalign(size_t, size_t);

void* hlvalloc(size_t);

int hlmallopt(int, int);

size_t hlmalloc_footprint(void);

size_t hlmalloc_max_footprint(void);

#if !NO_MALLINFO

struct mallinfo hlmallinfo(void);
#endif /* NO_MALLINFO */
void** hlindependent_calloc(size_t, size_t, void**);

void** hlindependent_comalloc(size_t, size_t*, void**);

void*  hlpvalloc(size_t);

int  hlmalloc_trim(size_t);

size_t hlmalloc_usable_size(void*);

void  hlmalloc_stats(void);
...
void** mspace_independent_calloc(void* msp, size_t n_elements, size_t elem_size, void* chunks[]);

void** mspace_independent_comalloc(void* msp, size_t n_elements, size_t sizes[], void* chunks[]);

size_t mspace_footprint(void* msp);

size_t mspace_max_footprint(void* msp);


#if !NO_MALLINFO

struct mallinfo mspace_mallinfo(void* msp);
#endif

void mspace_malloc_stats(void* msp);
...
void* hlcalloc(size_t n_elements, size_t elem_size)
{
  void* mem;
  size_t req = 0;
  if (n_elements != 0)
...
void* hlrealloc(void* oldmem, size_t bytes)
{
  if (oldmem == 0)
  ...
  {
#if !FOOTERS
    struct malloc_state* m = gm;
#else
    struct malloc_state* m = get_mstate_for(mem2chunk(oldmem));
    if(!ok_magic(m))
    {
      USAGE_ERROR_ACTION(m, oldmem);
      return 0;
...
void* hlmemalign(size_t alignment, size_t bytes)
{
  return internal_memalign(gm, alignment, bytes);
}

void** hlindependent_calloc(size_t n_elements, size_t elem_size, void* chunks[])
{
  size_t sz = elem_size;
  return ialloc(gm, n_elements, &sz, 3, chunks);
}

void** hlindependent_comalloc(size_t n_elements, size_t sizes[], void* chunks[])
{
  return ialloc(gm, n_elements, sizes, 0, chunks);
}

void* hlvalloc(size_t bytes)
{
  size_t pagesz;
  init_mparams();
...
size_t hlmalloc_footprint(void) { return gm -> footprint; }

size_t hlmalloc_max_footprint(void) { return gm -> max_footprint; }

#if !NO_MALLINFO
struct mallinfo hlmallinfo(void) { return internal_mallinfo(gm); }
#endif

void hlmalloc_stats() { internal_malloc_stats(gm); }

size_t hlmalloc_usable_size(void* mem)
{
  if (mem != 0)
  {
    mchunkptr p = mem2chunk(mem);
...
int hlmallopt(int param_number, int value) {
...
void* mspace_realloc(void* msp, void* oldmem, size_t bytes)
{
  if (oldmem == 0)
