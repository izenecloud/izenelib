...
class SkipListNode : boost::noncopyable {
 ...
 public:
  ...
  static SkipListNode* create(int& height, const U& data, bool isHead = false) {
    DCHECK(height >= 1 && height < 64) << height;

    size_t size = sizeof(SkipListNode) +
      height * sizeof(boost::atomic<SkipListNode*>);
    BOOST_AUTO( node , static_cast<SkipListNode*>(malloc(size)));
    // do placement new
    new (node) SkipListNode(height, boost::forward<U>(data), isHead);
    return node;
  }

  template<typename U>//=typename boost::enable_if_c<boost::is_convertible<U, T>::value>::type
  static SkipListNode* create(int& height, const U& data, bool isHead = false) {
    DCHECK(height >= 1 && height < 64) << height;
...
/**/
  ...
  inline SkipListNode* skip(int layer) const {
    ...
    return skip_[layer].load(boost::memory_order_consume);
  }
  ...
  void setSkip(uint8_t h, SkipListNode* next) {
    DCHECK_LT(h, height_);
    skip_[h].store(next, boost::memory_order_release);
  }
  ...
  boost::unique_lock<MicroSpinLock> acquireGuard() {
    return boost::unique_lock<MicroSpinLock>(spinLock_);
  }
 ...
 private:
      ...
      data_(boost::forward<U>(data)) 
  {
    ...
    for (uint8_t i = 0; i < height_; ++i) {
      new (&skip_[i]) boost::atomic<SkipListNode*>(nullptr);
    }
  }
/*rvref*/
  ~SkipListNode() {
  ...
  uint16_t getFlags() const {
    return flags_.load(boost::memory_order_consume);
  }
  void setFlags(uint16_t flags) {
    flags_.store(flags, boost::memory_order_release);
  }
  ...
  // MicroSpinLock.
  boost::atomic<uint16_t> flags_;
  const uint8_t height_;
  MicroSpinLock spinLock_;

  value_type data_;

  boost::atomic<SkipListNode*> skip_[0];
};
