...
#define FOR_EACH(i, c)                              \
  if (bool FOR_EACH_state1 = false) {} else         \
    for ( BOOST_AUTO(const & FOR_EACH_state2 , (c));              \
         !FOR_EACH_state1; FOR_EACH_state1 = true)  \
      for ( BOOST_AUTO( i , FOR_EACH_state2.begin());        \
...
class HasLess {
  ...
  template <typename C, typename D> static char test(BOOST_TYPEOF(C() < D())*);
...
template <class T, class U>
typename boost::enable_if_c< ( ( (boost::is_arithmetic<T>::value) && ( boost::is_arithmetic<U>::value ) ) || ( (boost::is_pointer<T>::value ) && (boost::is_pointer<U>::value) ) ),bool >::type
notThereYet(T& iter, const U& end) {
  return iter < end;
}
...
template <class T, class U>
typename boost::enable_if_c<HasLess<U, T>::value, bool>::type
...
#define FOR_EACH_RANGE(i, begin, end)           \
  for (BOOST_AUTO( i , (true ? (begin) : (end)));       \
...
#define FOR_EACH_RANGE_R(i, begin, end) \
  for (BOOST_AUTO (i , (false ? (begin) : (end))); ::folly::detail::downTo(i, (begin));)
