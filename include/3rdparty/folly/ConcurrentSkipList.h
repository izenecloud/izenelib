...
class ConcurrentSkipList {
  ...
  BOOST_STATIC_ASSERT_MSG(MAX_HEIGHT >= 2 && MAX_HEIGHT < 64,
      "MAX_HEIGHT can only be in the range of [2, 64)");
  typedef boost::unique_lock<folly::MicroSpinLock> ScopedLocker;
  typedef ConcurrentSkipList<T, Comp, MAX_HEIGHT> SkipListType;
  struct SkipListTypeDeleter {
    void operator()(SkipListType *p) {p->~ConcurrentSkipList();}
  };
 public:
  ...
  static boost::interprocess::unique_ptr<SkipListType,SkipListTypeDeleter> createRawInstance(int height=1) {
    return boost::interprocess::unique_ptr<SkipListType,SkipListTypeDeleter>(new SkipListType(height));
  }
  ...
  ~ConcurrentSkipList() {
    ...
    while (NodeType* current = head_.load(boost::memory_order_relaxed)) {
      NodeType* tmp = current->skip(0);
      NodeType::destroy(current);
      head_.store(tmp, boost::memory_order_relaxed);
    }
  }
 ...
 private:
  ...
  struct Recycler : private boost::noncopyable {
    ...
    ~Recycler() {
      if (nodes_) {
       // for (auto& node : *nodes_) {
        for(BOOST_AUTO(node , nodes_->begin()); node != nodes_->end(); ++node) {
    ...
    void add(NodeType* node) {
      boost::lock_guard<MicroSpinLock> g(lock_);
      ...
      DCHECK_GT(refs(), 0);
      dirty_.store(true, boost::memory_order_relaxed);
    }
    ...
    int addRef() {
      return refs_.fetch_add(1, boost::memory_order_relaxed);
    }
    ...
    int release() {
      ...
      if (LIKELY(!dirty_.load(boost::memory_order_relaxed) || refs() > 1)) {
        return refs_.fetch_add(-1, boost::memory_order_relaxed);
      }
      ...
      boost::scoped_ptr<std::vector<NodeType*> > newNodes;
      {
        ...
        dirty_.store(false, boost::memory_order_relaxed);
      }

      // TODO(xliu) should we spawn a thread to do this when there are large
      // number of nodes in the recycler?
      //for (auto& node : *newNodes)
      for(BOOST_AUTO(node , newNodes->begin()); node != newNodes->end(); ++node) {
        NodeType::destroy(*node);
      }
   ...
   private:
    ...
    boost::atomic<int32_t> refs_; // current number of visitors to the list
    boost::atomic<bool> dirty_; // whether *nodes_ is non-empty
  ...
  size_t size() const { return size_.load(boost::memory_order_relaxed); }
  int height() const {
    return head_.load(boost::memory_order_consume)->height();
  }
  int maxLayer() const { return height() - 1; }

  size_t incrementSize(int delta) {
    return size_.fetch_add(delta, boost::memory_order_relaxed) + delta;
  }

  // Returns the node if found, nullptr otherwise.
  NodeType* find(const value_type &data) {
    BOOST_AUTO( ret , findNode(data));
  ...
  std::pair<NodeType*, size_t> addOrGetData(U &data) {
    ...
    while (true) {
      ...
      newNode = NodeType::create(nodeHeight, boost::forward<U>(data));
...
/**/
  ...
  const value_type *first() const {
    BOOST_AUTO( node , head_.load(boost::memory_order_consume)->skip(0));
    return node ? &node->data() : nullptr;
  }

  const value_type *last() const {
    NodeType *pred = head_.load(boost::memory_order_consume);
    ...
    return pred == head_.load(boost::memory_order_relaxed)
      ? nullptr : &pred->data();
  }
  ...
  int findInsertionPointGetMaxLayer(const value_type &data,
    ...
    *max_layer = maxLayer();
    return findInsertionPoint(head_.load(boost::memory_order_consume),
  ...
  NodeType* lower_bound(const value_type &data) const {
    BOOST_AUTO( node , findNode(data).first);
  ...
  void growHeight(int height) {
    NodeType* oldHead = head_.load(boost::memory_order_consume);
  ...
  boost::atomic<NodeType*> head_;
  Recycler recycler_;
  boost::atomic<size_t> size_;
};
...
class ConcurrentSkipList<T, Comp, MAX_HEIGHT>::Accessor {
 ...
 public:
  ...
  explicit Accessor(boost::shared_ptr<ConcurrentSkipList> skip_list)
    : slHolder_(boost::move(skip_list))
  {
    sl_ = slHolder_.get();
    DCHECK(sl_ != nullptr);
  ...
  iterator begin() const {
    NodeType* head = sl_->head_.load(boost::memory_order_consume);
  ...
  template<typename U, typename>//  typename=typename boost::enable_if_c<boost::is_convertible<U, T>::value>::type>
  std::pair<iterator, bool> insert(const U& data) {
    BOOST_AUTO( ret , sl_->addOrGetData(boost::forward<U>(data)));
    return std::make_pair(iterator(ret.first), ret.second);
  }

  template<typename U>//  typename=typename boost::enable_if_c<boost::is_convertible<U, T>::value>::type>
  std::pair<iterator, bool> insert(const U& data) {
    BOOST_AUTO( ret , sl_->addOrGetData(boost::forward<U>(data)));
    return std::make_pair(iterator(ret.first), ret.second);
  }
...
class detail::csl_iterator :
 ...
 public:
  ...
  csl_iterator(const csl_iterator<OtherVal, OtherNode> &other,
      typename boost::enable_if_c<boost::is_convertible<OtherVal, ValT>::value>::type*
      = 0) : node_(other.node_) {}
...
class ConcurrentSkipList<T, Comp, MAX_HEIGHT>::Skipper {
 ...
 private:
  NodeType* head() const {
    return accessor_.skiplist()->head_.load(boost::memory_order_consume);
  }
