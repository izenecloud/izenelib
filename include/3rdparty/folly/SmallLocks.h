...
struct PicoSpinLock {
  ...
  typedef typename boost::make_unsigned<IntType>::type UIntType;

  BOOST_STATIC_ASSERT_MSG(boost::is_integral<IntType>::value,
                "PicoSpinLock needs an integral type");
  BOOST_STATIC_ASSERT_MSG(sizeof(IntType) == 2 || sizeof(IntType) == 4 ||
                  sizeof(IntType) == 8,
...
struct SpinLockArray {
 ...
 private:
  ...
  BOOST_STATIC_ASSERT_MSG(sizeof(PaddedSpinLock) == FOLLY_CACHE_LINE_SIZE,
...
/*
  BOOST_STATIC_ASSERT_MSG(alignof(MaxAlign) > 0 &&
                FOLLY_CACHE_LINE_SIZE % alignof(MaxAlign) == 0 &&
                sizeof(T) <= alignof(MaxAlign),
                "T can cross cache line boundaries");
*/
  char padding_[FOLLY_CACHE_LINE_SIZE];
  boost::array<PaddedSpinLock, N> data_;
} __attribute__((aligned));

//////////////////////////////////////////////////////////////////////

typedef boost::lock_guard<MicroSpinLock> MSLGuard;
