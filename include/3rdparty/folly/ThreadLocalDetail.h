...
#ifndef FOLLY_TLPDestructionMode
#define FOLLY_TLPDestructionMode
namespace folly {
...
class DeleterBase {
 public:
  virtual ~DeleterBase() { }
  virtual void dispose(void* ptr, enum TLPDestructionMode mode) const = 0;
};
...
class SimpleDeleter : public DeleterBase {
 public:
  virtual void dispose(void* ptr, enum TLPDestructionMode mode) const {
...
struct ElementWrapper {
  ...
  void set(Ptr p) {
    ...
    if (p) {
     ...
     BOOST_AUTO( d ,new SimpleDeleter<Ptr>());
      ptr = p;
      deleter = d;
      ownsDeleter = false;
    }
...
struct StaticMeta {
  ...
  static void onThreadExit(void* ptr) {
    ...
    FOR_EACH_RANGE(i, 0, threadEntry_.elementsCapacity) {
      threadEntry_.elements[i].dispose(THIS_THREAD);//TLPDestructionMode::
    }
    ...
    try {
      ...
      std::vector<ElementWrapper> elements;
      {
        ...
        for (ThreadEntry* e = meta.head_.next; e != &meta.head_; e = e->next) {
          if (id < int(e->elementsCapacity) && e->elements[id].ptr) {
      ...
      for ( BOOST_AUTO( it , elements.begin());  it !=elements.end(); ++it)
      {
        it->dispose(ALL_THREADS);//TLPDestructionMode::
      }
