...
class RWSpinLock : boost::noncopyable {
  enum kType{ READER = 4, UPGRADED = 2, WRITER = 1 };
 public:
  RWSpinLock() : bits_(0) {}
  ...
  void unlock() {
    BOOST_STATIC_ASSERT_MSG(READER > WRITER + UPGRADED, "wrong bits!");
    bits_.fetch_and(~(WRITER | UPGRADED), boost::memory_order_release);
  }
  ...
  void unlock_and_lock_shared() {
    bits_.fetch_add(READER, boost::memory_order_acquire);
    unlock();
  }
  ...
  void unlock_upgrade() {
    bits_.fetch_add(-UPGRADED, boost::memory_order_acq_rel);
  }
  ...
  void unlock_and_lock_upgrade() {
    ...
    bits_.fetch_or(UPGRADED, boost::memory_order_acquire);
    bits_.fetch_add(-WRITER, boost::memory_order_release);
  }
  ...
  bool try_lock_upgrade() {
    int32_t value = bits_.fetch_or(UPGRADED, boost::memory_order_acquire);
  ...
  int32_t bits() const { return bits_.load(boost::memory_order_acquire); }
  ...
  class UpgradedHolder : boost::noncopyable{
   public:
...
template <size_t kBitWidth> struct RWTicketIntTrait {
  BOOST_STATIC_ASSERT_MSG(kBitWidth == 32 || kBitWidth == 64,
