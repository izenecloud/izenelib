...
#define MID_RPC_INTERFACE END_RPC_ADD_MF

#define END_RPC_INTERFACE() };
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


BOOST_STATIC_CONSTANT(unsigned, rpc_call_asynch_noorder	= 1);
BOOST_STATIC_CONSTANT(unsigned, rpc_call_asynch_ordered = 2);
BOOST_STATIC_CONSTANT(unsigned, rpc_call_synch  = 3);
...
/**
 @brief 输入参数
 在 client 仅发送，不接收 @see client_io.h
 在 server 仅接收，不发送 @see server_io.h
 */
...
/**
 @brief 输出参数
 在 client 仅接收，不发送 @see client_io.h
 在 server 仅发送，不接收 @see server_io.h
 */
...
/**
 @brief 输入/输出参数
 在 client/server 既发送，又接收 @see client_io.h, server_io.h
 */
...
public:
	...
	void* get_ext_ptr() const
	{ 
		return (void*)(m_flag_ptr & ~(uintptr_t)3);
	}
