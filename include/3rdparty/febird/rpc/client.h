...
class rpc_client_basebase;

struct FEBIRD_DLL_EXPORT client_packet_base : public RefCounter
{
...
{
public:
	typedef arglist_ref<Function> arefs_t;

	rpc_ret_t t_call_byid(rpc_client_basebase* vpclient, arefs_t& refs)
	{
		Client* client = (Client*)(vpclient);
		this->how_call = rpc_call_synch;
		if (0 == this->stub->m_callid)
		...
		var_uint32_t vseqid1;
		client->m_co_input.get() >> vseqid1;
		assert(this->seqid == vseqid1.t);
		return this->retv;
	}
	rpc_ret_t t_call_byname(rpc_client_basebase* vpclient, arefs_t& refs)
	{
		Client* client = (Client*)(vpclient);
		this->how_call = rpc_call_synch;
		t_send_nm_args(client, refs);
	...
	void t_read_args(rpc_client_basebase* vpclient, arefs_t& refs)
	{
		Client* client = (Client*)(vpclient);
	...
	void t_send_id_args(rpc_client_basebase* vpclient, arefs_t& refs)
	{
		this->isbyid = true;
		Client* client = (Client*)(vpclient);
		...
		client->m_co_output.get()
			<< var_uint32_t(this->seqid)
			<< var_uint32_t(this->stub->m_callid)
			<< var_uint32_t(this->how_call)
			;
		refs.save(client->m_co_output);
		client->m_co_output.flush();
	}
	void t_send_nm_args(rpc_client_basebase* vpclient, arefs_t& refs)
	{
		this->isbyid = false;
		Client* client = (Client*)(vpclient);
		boost::mutex::scoped_lock lock(client->m_mutex_write);
...
class FEBIRD_DLL_EXPORT rpc_client_basebase : public SessionScope
{
protected:
	std::map<std::string, client_stub_i*> m_stubTable;
//	CycleQueue<boost::intrusive_ptr<client_packet_base> > m_pendings;
...
public:
	...
	explicit rpc_client_basebase(IDuplexStream* duplex);
	virtual ~rpc_client_basebase();

	void put_packet(boost::intrusive_ptr<client_packet_base> p);
...
#define BOOST_PP_FILENAME_1       <febird/rpc/pp_client_stub.h>
...
struct client_stub : public client_stub_i
{
	...
	explicit client_stub(const std::string& name)
	{
		m_name = name;
	}
...
class FEBIRD_DLL_EXPORT rpc_client_base : public rpc_client_basebase 
{
protected:
