...
template<typename PrimaryCompressor, typename SecondaryCompressor, int BLOCK_SIZE,  int MIN_PADDING_SIZE >
...
public:
    ...
    int compress(uint32_t* input, uint32_t* output, int num_input_elements) const
    {
        ...
        while (num_whole_blocks-- > 0)
        {
            ...
            unencoded_offset += BLOCK_SIZE;
        }

        int left_to_encode = num_input_elements % BLOCK_SIZE;
        if (left_to_encode == 0)
        {
            // Nothing to do here.
        }
        ...
        else
        {
            ...
            // Assumption here is that the 'input' array size is at least an upper multiple of 'BLOCK_SIZE'.
            int pad_until = BLOCK_SIZE * ((num_input_elements / BLOCK_SIZE) + 1);
    ...
    int decompress(uint32_t* input, uint32_t* output, int num_input_elements) const
    {
        ...
        while (num_whole_blocks-- > 0)
        {
            encoded_offset += primary_coder_->decompress(input + encoded_offset, output + unencoded_offset, BLOCK_SIZE);
            unencoded_offset += BLOCK_SIZE;
        }
        ...
        else
        {
            ...
            // Assumption here is that the 'output' array size is at least an upper multiple of 'BLOCK_SIZE'.
            encoded_offset += primary_coder_->decompress(input + encoded_offset, output + unencoded_offset, BLOCK_SIZE);
        }
