...
static const char* indexFile = "indexsdb23432.dat";
...
namespace util {
	...
	template<> inline void read_image< iKeyType<KeyType> >(iKeyType<KeyType>& dat,
			const DbObjPtr& ptr) {
	...
	template<> inline void write_image< iKeyType<KeyType> >(
			const iKeyType<KeyType>& dat, DbObjPtr& ptr) {
		stringstream ostr;
		{
	...
	template<> inline void read_image<std::vector<DocID> >(std::vector<DocID>& dat,
			const DbObjPtr& ptr) {
		stringstream istr((char*)ptr->getData());
		{
			boost::archive::binary_iarchive ia(istr);
	...
	template<> inline void write_image<std::vector<DocID> >(
			const std::vector<DocID>& dat, DbObjPtr& ptr) {
		stringstream ostr;
		{
			boost::archive::binary_oarchive oa(ostr);
			oa & dat;
...
BOOST_AUTO_TEST_CASE(t_IndexSDB)
{
