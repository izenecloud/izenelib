        ...
        size_t tailPointer)
{
    ...
    codec_.encodeArray(docBlock, BLOCK_SIZE, &segment[2], csize);
    segment[1] = csize;

    size_t scsize = BLOCK_SIZE * 2;
    codec_.encodeArray(scoreBlock, BLOCK_SIZE, &segment[csize + 3], scsize);
    segment[csize + 2] = scsize;

    return pool_.appendSegment(segment, maxDocId, csize + scsize + 3, tailPointer);
}
        ...
        std::vector<uint32_t>& score_list) const
{
    ...
    if (algorithm == SVS)
    {
        intersectSvS_(qHeadPointers, filter, minimumDf, hits, docid_list, score_list);
    }
}
