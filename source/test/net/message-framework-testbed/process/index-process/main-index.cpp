...
int registerServiceList(const vector<ServiceInfo> & serviceList) {
	...
	for (i = 0; i < serviceList.size(); i++) {
		while ( !(ret = server_->registerService(serviceList[i]) ) && nTimeout
				< 100) {
			cerr << "[IndexProcess] service registration failed, trying again."
					<< endl;
...
int indexServerMain() {
	...
	while (true) {
        ...
        //cout<<" requestList idx= "<<i<<endl;
			...
			else if (requestList[i]->getServiceName() == "findDocListByTerm")
...
MF_AUTO_MAIN(indexProcess) {
	...
	indexServerMain();
	//boost::thread serverThread( &indexServerMain );
	//serverThread.join();
