...
namespace DocumentProcess
{
	   ...
	   void  getDocument(const DocumentManager& docMgr, ServiceMessagePtr& sm)
   {
	...
	bool buildForwardIndex( const vector<Document> & docList, vector<ForwardIndex> & forwardIndexList );
      ...
      posix_time::ptime after = posix_time::microsec_clock::local_time();
	   ...
	   cout << "[PROFILE] on client la cost " << laTime << " macroseconds" << endl;
		...
		for( int i=0; i < docList.size(); i++ )
		{
			...
			requestsToIndex[i] = shared_ptr<ServiceRequestInfo>(new ServiceRequestInfo);
		...
		if( false == requestService("addDocument", requestsToIndex, *client_) )
		  return false;
		after = posix_time::microsec_clock::local_time();
		indexTime += (after - before).total_microseconds();
		
		cout << "[PROFILE] on client index cost " << indexTime << " macroseconds" << endl;
	...
	int registerServiceList( const vector<ServiceInfo> & serviceList )
	{
		...
		for( i = 0; i < serviceList.size(); i++ )
		{
			...
			{
				cout << "[DocumentProcess] service registration failed, trying again." << endl;
	...
	int docServerMain()
	{
		...
		{
			cerr << "[DocumentProcess]: number of failed service registrations: " << ret << endl;
			return -1;
		}
...
//				serviceResult.clear();
			...
			cout << "[PROFILE] doc process " << "total time" << docTime  << "ms" << endl;
		}
	}
