...
MF_AUTO_MAIN( mainProcess )
{
    ...
    MessageFrameworkNode controllerInfo( po.getControllerIp(), po.getControllerPort() );
...
//        client_->getMessageDispatcher().deserialization_time = 0;
       ...
       // requestToLA.appendParameter(queryString);
        ...
        if( ret == false )
        {
            cerr << "[Main-Process]: ERROR when calling service, \"parseString\" " << endl;
            return -1;
        }
        else
        {
        ...
        for( size_t i = 0; i < termList->size(); i++ )
        {
            ...
            {
                cout << "\tDEBUG [Main-Process]: docIdList.size(): " << docIdList->size() << endl;
            }
           ...
           // requestsToDoc.resize(docIdList->size());
            ...
            if( false == requestService( "getDocument", requestsToDoc, resultsFromDoc, *client_) )
            {
				cout << "[Main-Process]: ERROR when calling service \"getDocument\"" << endl;
				return -1;
            }
            else
            {
        ...
        cout << "[PROFILE] get permission cost " << messageframework::permission_time << " microseconds" << endl;
        cout << "[PROFILE] send message cost " << messageframework::send_time << " microseconds" << endl;
        cout << "[PROFILE] recv result cost " << messageframework::recv_time << " microseconds" << endl;
