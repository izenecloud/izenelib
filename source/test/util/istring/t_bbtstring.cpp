...
#include <iostream>
#include <util/istring/bbt_string.hpp>
#include <vector>
#include <boost/archive/binary_iarchive.hpp>
#include <boost/archive/binary_oarchive.hpp>
#include <fstream>
#include <util/istring/tstring>

using namespace std;
using namespace izenelib::util;

BOOST_AUTO_TEST_SUITE( t_bbt_string_suite )
...
typedef bbt_string<CharT, 1, BUCKET_BYTES> istring;
...
BOOST_AUTO_TEST_CASE(izene_istring_modifiers_check)
{
  ...
  t++;
  s = ss;
  s.push_front(conv("UUUUUUUU",buf));
  s.push_front(conv("TTTTTTTT",buf));
  if (s.compare(conv("TTTTTTTTUUUUUUUUsfdfsfssfssfskkdfs",buf))!=0)
    cout<<"["<<t<<"] ERROR\n";
...
const size_t bb = (size +1)*sizeof(CharT);
