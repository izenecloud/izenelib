 ...
 */
rpc_client_basebase::rpc_client_basebase(IDuplexStream* duplex)
	: m_minseqid(0)
	, m_sequence_id(0)
	, m_duplex(duplex)
{
...
rpc_client_basebase::~rpc_client_basebase()
{
 ...
 */
void rpc_client_basebase::put_packet(boost::intrusive_ptr<client_packet_base> p)
{
	boost::mutex::scoped_lock lock(m_mutex_pendings);
	m_pendings.insert(std::make_pair(p->seqid, p));
}

void rpc_client_basebase::wait_pending_async()
{
...
void rpc_client_basebase::wait_async_return_once()
{
	boost::intrusive_ptr<client_packet_base> packet;
	{
		boost::mutex::scoped_lock lock(m_mutex_read);
...
/**
 @brief 轮询等待未返回的rpc调用
 */
void rpc_client_basebase::wait_async_return()
{
	m_run = true;
	while (m_run)
	{
...
//! 开启异步调用的等待线程
void rpc_client_basebase::start_async(int nThreads)
{
	assert(nThreads > 0);
	m_run = 1;
	...
	{
		m_threads[i].reset(new boost::thread(boost::bind(&rpc_client_base::wait_async_return, this)));
	}
}

rpc_client_base::rpc_client_base(IDuplexStream* duplex) : rpc_client_basebase(duplex)
{
}
