...
//	assert(m_pos + length > m_end);
    ...
    while (n2 > 0)
    {
        ...
        size_t readed = m_is->zcRead((const void**)&m_beg, toRead);
    ...
    for (;;)
    {
        for (byte* p = m_pos; ; ++p, ++len)
        {
            ...
            if (febird_unlikely(p == m_end))
            {
                ...
                size_t nRead = m_is->zcRead((const void**)&m_beg, m_bufsize);
                m_end = m_beg + nRead;
                m_pos = m_beg;
                if (0 == nRead)
                {
...
size_t ZcWriter::do_flush_and_write(const void* vbuf, size_t length)
{
    ...
    while (n2 > 0)
    {
        ...
        size_t writable = m_os->zcWrite((void**)&m_beg, toWrite);
