...
static void ms_get_serverlist(char *str)
{
  ...
  uint32_t loop;
  for (loop= 0; loop < memcached_server_list_count(server_pool); loop++)
  {
...
static void ms_calc_avg_size()
{
  ...
  int j;
  for (j= 0; j < ms_setting.val_rng_cnt; j++)
  {
  ...
  for (j= 0; j < ms_setting.key_rng_cnt; j++)
  {
...
static void ms_build_distr()
{
  ...
  {
    int i;
    for (i= 0; i < units * UNIT_ITEMS_COUNT; i++)
    {
...
static void ms_init_random_block()
{
  ...
  int i;
  for (i= 0; (size_t)i < ms_setting.char_blk_size; i++)
  {
